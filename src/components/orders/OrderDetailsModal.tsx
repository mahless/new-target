...
export const OrderDetailsModal: React.FC<OrderDetailsModalProps> = ({
  ...
  const orderPayments = payments.filter(p => matchIds(p.order_id, order.id));
  const creationBranch = branches.find(b => matchIds(b.id, order.creation_branch_id));
  const currentBranch = branches.find(b => matchIds(b.id, order.current_branch_id));
  const deliveryBranch = branches.find(b => matchIds(b.id, order.delivery_branch_id));
  const createdByEmp = employees.find(e => matchIds(e.id, order.created_by_employee_id));
  const distributor = distributors.find(d => matchIds(d.id, order.distributor_id));
  const externalOffice = externalOffices.find(o => matchIds(o.id, order.external_office_id));
