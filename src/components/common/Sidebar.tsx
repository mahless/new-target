...
export const Sidebar: React.FC = () => {
  ...
  const allowedItems = navItems.filter(item => {
    if (role === 'viewer') {
      return !['new_service', 'treasury', 'transfers', 'daily_closing', 'settings', 'audit_logs', 'reports'].includes(item.id);
    }
    if (role === 'employee') {
      return !['treasury', 'settings', 'audit_logs', 'reports'].includes(item.id);
    }
